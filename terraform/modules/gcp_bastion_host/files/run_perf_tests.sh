...
function run_kms_tests(){
  ...
  /usr/pgsql-15/bin/pgbench --log --protocol=extended --report-per-command --jobs=5 --client=50 --time=600 --host="${database_kms_host}" --username="${database_kms_username}" ${database_kms_name}
}
...
function run_ekm_tests(){
  ...
  # Run performance tests
  /usr/pgsql-15/bin/pgbench --log --protocol=extended --report-per-command --jobs=5 --client=50 --time=600 --host="${database_ekm_host}" --username="${database_ekm_username}" ${database_ekm_name}
}
